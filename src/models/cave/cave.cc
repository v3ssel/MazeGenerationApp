...
bool Cave::StepRender(unsigned die_limit, unsigned born_limit) {
    ...
    for (int row = 0; row < rows_; row++) {
        ...
        for (int col = 0; col < cols_; col++) {
            ...
            ApplyRules(index, GetAliveNeigborsCount(&old_cave, current_row,
                                                    current_col));
        }
    }
...
bool Cave::CompareCaves(Cave *old_cave, Cave *new_cave) {
    ...
    for (int i = 0; i < old_cave->GetRows(); i++) {
        for (int j = 0; j < old_cave->GetCols(); j++) {
            bool old_cave_value =
                old_cave->GetMatrix()[i * old_cave->GetCols() + j];
            bool new_cave_value =
                new_cave->GetMatrix()[i * new_cave->GetCols() + j];
...
int Cave::GetAliveNeigborsCount(Cave *cave, int r, int c) {
    ...
    int right =
        (((c + 1) >= cols_) ? 1 : cave->GetMatrix()[r * cols_ + (c + 1)]);
