        ...
        for (auto direction : directions) {
            ...
            if (neighbor_cell >= 0 && neighbor_cell < (int)matrix_.size() &&
                ...
                Border current_border =
                    matrix_[current_row * cols_ + current_col];
                Border neighbor_border =
                    matrix_[neighbor_row * cols_ + neighbor_col];

                if (CheckBorder(direction, current_border, neighbor_border,
                                current_row, current_col, neighbor_row,
                                neighbor_col)) {
                    continue;
                }
...
bool Maze::CheckBorder(int dir, int c_b, int n_b, int c_r, int c_c, int n_r,
    ...
    if (dir == -1 &&
        (n_r != c_r || (n_b == RIGHT_BORDER || n_b == BOTH_BORDER)))
        answer = true;
    if (dir == -cols_ &&
    ...
    while (!q.empty()) {
        ...
        for (auto direction : directions) {
            ...
            if (neighbor_cell >= 0 && neighbor_cell < matrix_size &&
                ...
                int neighbor_col = neighbor_cell % cols_;
                Border current_border =
                    GetMatrix()[current_row * cols_ + current_col];
                Border neighbor_border =
