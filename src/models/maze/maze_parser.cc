...
void MazeParser::MergeMatricies() {
    ...
    if (bottom_matrix_.size() !=
        (size_t)(maze_->GetCols() * maze_->GetRows())) {
        Reset();
        throw std::invalid_argument("Second matrix size mismatch: " +
                                    filepath_);
    }
