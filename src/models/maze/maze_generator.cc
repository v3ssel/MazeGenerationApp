...
Maze MazeGenerator::GenerateMaze(int rows, int cols, float right_wall_chance,
                                 float bottom_wall_chance) {
...
void MazeGenerator::AddRightWalls(Maze& maze, int row,
...
void MazeGenerator::AddBottomWalls(Maze& maze, int row,
                                   float bottom_wall_chance) {
    for (size_t cell = 0; cell < static_cast<size_t>(maze.GetCols()); cell++) {
        int cur_cell = cells_[cell];
        size_t bottom_borders = std::count_if(
            sets_[cur_cell].begin(), sets_[cur_cell].end(),
            [&maze, row](int c) {
                return maze.GetMatrix()[row * maze.GetCols() + c] ==
                           BOTTOM_BORDER ||
                       maze.GetMatrix()[row * maze.GetCols() + c] ==
                           BOTH_BORDER;
            });
        ...
        if (Utils::RandomChoice(bottom_wall_chance) &&
            ...
            sets_[cur_cell].erase(std::find(sets_[cur_cell].begin(),
                                            sets_[cur_cell].end(), cell));
...
void MazeGenerator::MergeSets(size_t current_set_id, size_t target_set_id) {
    for (auto target_cell : sets_[target_set_id]) {
        ...
        vec.insert(std::upper_bound(vec.begin(), vec.end(), target_cell),
                   target_cell);
    }
